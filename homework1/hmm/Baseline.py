    ...
    def train(self, corpus_file):
        ...
        for ngram in ngram_iterator:
            ...
            for word, ne_tag in self.emission_counts:
                self.cpd[(word, ne_tag)] = self.emission_counts[(word, ne_tag)] / self.ngram_counts[0][(ne_tag,)]
...
if __name__ == "__main__":
    ...
    f_tagged = file('./gene_test.p1.out', 'w')
    counter = Hmm()
    counter.read_counts(f_counts)
    f_counts.close()
    ...
    while l:
        word = l.strip()
        if word != '':
            tagged_for_word = max(counter.all_states, key=lambda x: counter.cpd[(word,x)])
            _l = word + " " + tagged_for_word + "\n"
        else:
            _l = "\n"
        f_tagged.write(_l)
        l = f_test.readline()